...
export function filterIncompleteResults(results) {
  return fromJS(results).filter(
    movie => movie.get('overview') && movie.get('backdrop_path')
  );
}

export function findGenreName(genreId, genres) {
  const obj = genres.find(
    genreObj => genreObj.get('id') === genreId,
    null,
    Map()
  );
...
export function getGenresAndDecades(results, genres) {
  const decades = results
    .map(movie => movie.get('release_date', '').slice(0, 3))
    .sort()
    .toOrderedSet();

  const resultGenres = results
...
export function getArgs(sagaObject) {
  const args = Object.keys(sagaObject).reduce((prev, current) => {
    if (current === 'args') {
  ...
  const type = Object.keys(sagaObject).reduce((prev, current) => {
