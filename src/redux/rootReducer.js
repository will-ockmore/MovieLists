...
export default function(state = initialState, action) {
  switch (action.type) {
    ...
    case sagaActions.GET_FURTHER_MOVIES.SUCCESS:
      ...
      const oldResultGenres = state.getIn([
        'responses',
        'movies',
        'resultGenres',
      ]);
      ...
      const decades = oldDecades.union(newDecades).sort();

      const resultGenres = oldResultGenres.union(newResultGenres).sort();

      return state.mergeIn(['responses', 'movies'], {
        results,
        decades,
        resultGenres,
      });
    ...
    case sagaActions.GET_MOVIE_DETAILS.REQUEST:
      return state.setIn(
        ['responses', 'movieDetails', 'status'],
        responseStates.PENDING
      );

    case sagaActions.GET_MOVIE_DETAILS.SUCCESS:
      return state.mergeIn(['responses', 'movieDetails'], {
        result: fromJS(action.payload),
        status: responseStates.SUCCESS,
      });

    case actions.CHANGE_DECADE_FILTER:
      return state.setIn(['filters', 'decade'], action.payload.value);

    case actions.CHANGE_GENRE_FILTER:
      return state.setIn(['filters', 'genre'], action.payload.value);
    ...
    case sagaActions.GET_BACKDROP_URL.SUCCESS:
      ...
      return state.mergeIn(['imageUrls'], {
        smallBackdropUrl,
        largeBackdropUrl,
        smallProfileUrl,
      });
