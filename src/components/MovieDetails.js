  ...
  render() {
    ...
    return (
      <div className="movie-details">
        <img
          className="splash"
          src={largeBackdropUrl + backdrop_path}
          alt="poster"
        />
        <h1 className="mv-title">{title}</h1>
        <h2>
          {original_title && original_title !== title && ` (${original_title})`}
        </h2>
        {tagline && <h4 className="tagline">“<em>{tagline}</em> ”</h4>}
        ...
        {hasMultipleVotes &&
          <PropertyRow propertyTitle="Rating:">
            ...
            /10 &nbsp;
            <small>(from {vote_count} votes)</small>
          </PropertyRow>}
        ...
        {crew &&
          crew.slice(0, 5).map(crewmember => (
            <PropertyRow
              key={crewmember.get('credit_id')}
              propertyTitle={crewmember.get('job')}
            >
              {crewmember.get('name')}
            </PropertyRow>
          ))}
        ...
        <div className="mt-2">
          {cast &&
            cast.slice(0, 15).map(castmember => (
              <PropertyRow
                key={castmember.get('cast_id')}
                propertyTitle={castmember.get('character')}
              >
                {castmember.get('name')}
              </PropertyRow>
            ))}
